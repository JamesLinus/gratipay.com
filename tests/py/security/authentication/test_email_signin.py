...
class TestCreateSigninNonce(Harness):
    ...
    def test_inserts_into_db(self):
        nonce = create_signin_nonce(self.db, 'alice@gratipay.com')
    ...
    def test_email_id_is_properly_referenced(self):
        nonce = create_signin_nonce(self.db, 'alice@gratipay.com')
        email_id = self.db.one("SELECT id FROM emails WHERE address = %s", ('alice@gratipay.com', ))
        rec = self._fetch_rec_from_db(nonce)

        assert rec['email_id'] == email_id
    ...
    def test_complains_if_email_not_in_database(self):
        with pytest.raises(IntegrityError):
            create_signin_nonce(self.db, 'dummy@gratipay.com')
...
class TestVerifyNonce(Harness):
    ...
    def test_invalid_nonce(self):
        create_signin_nonce(self.db, 'alice@gratipay.com')
...
class TestInvalidateNonce(Harness):
    ...
    def test_only_deletes_one_nonce(self):
        nonce1 = create_signin_nonce(self.db, 'alice@gratipay.com')
        nonce2 = create_signin_nonce(self.db, 'alice@gratipay.com')
